...
author: "Pivot_longer(better, faster)"
date: "`r Sys.Date()`"
output:
...
```{r, warning = FALSE}
df_basic %>% ggplot(aes(education, compensation_USD, fill = education)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90))
...
summarize_df <- function(df){
  ...
  for (i in colNames){
    ...
    new_df <- subset(mega_df, mega_df[i] != 0)
    meds <- new_df %>% group_by_(i) %>% summarize(med = median(compensation_USD, na.rm = T), count = n())
  ...
  returndf <- data.frame('skill' = titles, 'medians' = med, 'counts' = counts)
  return(returndf)
}
```
...
df_prime_tool %>% inner_join(df_basic, by = 'id') %>% ggplot(aes(x = str_wrap(primary_analysis_tool, 12), y = compensation_USD, fill = primary_analysis_tool)) + geom_boxplot() + theme(axis.text.x = element_text(size = 6), legend.text = element_text(size = 6))

df_ml_xp %>% inner_join(df_basic, by = 'id') %>% ggplot(aes(x = factor(clean_numeric_bins(ml_exp_years)), y = compensation_USD, fill = factor(clean_numeric_bins(ml_exp_years)))) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90))

##Plot other side dataframes using function created.
for (i in df_list){
  winnerdf <- summarize_df(data.frame(i))
  winnerdf <- winnerdf %>% filter(!(skill %in% c('None', 'Other')))
  
  plots <- winnerdf %>% ggplot(aes(counts, medians, color = str_wrap(skill, 15))) + geom_point() + theme(legend.text = element_text(size = 5), legend.title = element_blank()) + xlab('Popularity (counts)') +ylab('Median Compensation')
  print(plots)
}
