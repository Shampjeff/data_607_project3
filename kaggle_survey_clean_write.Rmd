...
df_list <- list(df_media, df_online_ed, df_ide, df_lang, df_viz, df_ml_algo, df_ml_tool, df_cv, df_nlp, df_cloud, df_cloud_prod, df_big_data, df_cloud_ml, df_db)
```
...
plot_df <- function(df){
  ...
  colNames <- names(df %>% select(-id))
  
  ## First loop: iterate through the columns of the dataframe
  for (i in colNames){
    ## Calculate the median for each group in said column
    meds <- by(mega_df$compensation_USD, mega_df[i], median, na.rm = TRUE)
    if (length(meds) > 2){
      ## Iterate over each group in the column
      for (j in 1:length(meds)){
        ## If the group median is greater than the null median
        if (meds[[j]] > meds['0']){
            myplot <- mega_df %>% ggplot(aes_string(x = i, y = "compensation_USD", fill = i)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90))
            print(myplot)
            print(meds)
        }
      }
    }else{
      if (meds[[2]] > meds['0']|meds[[1]] > meds['0']){
        myplot <- mega_df %>% ggplot(aes_string(x = i, y = "compensation_USD", fill = i)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90))
          print(myplot)
          print(meds)
      }
...
```{r}
df_prime_tool %>% inner_join(df_basic, by = 'id') %>% ggplot(aes(x = primary_analysis_tool, y = compensation_USD, fill = primary_analysis_tool)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90))

by(df_basic$compensation_USD, df_prime_tool$primary_analysis_tool, median, na.rm = TRUE)

df_ml_xp %>% inner_join(df_basic, by = 'id') %>% ggplot(aes(x = ml_exp_years, y = compensation_USD, fill = ml_exp_years)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90))

by(df_basic$compensation_USD, df_ml_xp$ml_exp_years, median, na.rm = TRUE)

for (i in df_list){
  plot_df(data.frame(i))
}
...
Variables that effect compensation:
...
Cloud based data software (AWS, Azure, etc.)
Advanced stats software (sPSS, SAS)
...
Decision Trees
Gradient Boosting
Bayesian Approaches (Naive Bayes, etc.)
...
MicrosoftSQL
Oracle Database
AWS Relational Databse Service
Azure SQL Database
